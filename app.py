...
<style>
    ...
    /* CARD STYLING (360px Fixed Width) */
    .parlay-card {
        flex: 0 0 360px;
    ...
    if not mentioned_event:
        for event in events:
            if (event['teams']['home']['name'].lower() in lower or 
                event['teams']['away']['name'].lower() in lower):
                mentioned_event = event
                break
    if not mentioned_event: return None
    return {'event': mentioned_event, 'winning_team': None, 'is_high_scoring': 'over' in lower, 'players': []}
...
# 1. ALWAYS-VISIBLE COMPACT FILTERS + CHAT
with st.container():
    ...
    with f_col3:
        with st.form("p_form", clear_on_submit=False):
            u_input = st.text_input("Prompt", placeholder="Knicks win...", label_visibility="collapsed")
...
if st.session_state.recommendations:
    ...
    for idx, p in enumerate(st.session_state.recommendations):
        with cols[idx]:
            st.markdown(f"""<div class="parlay-card"><div style="display:flex; justify-content:space-between; margin-bottom:15px;"><span class="odds-badge">{p['odds_american']}</span><span style="color:#8e8e93;">{len(p['legs'])} Legs</span></div>""", unsafe_allow_html=True)
...
if st.session_state.selected_parlay:
    ...
    for leg in p['legs']:
        st.markdown(f"<div class='leg-item'><div style='font-weight:600;'>{leg['display']}</div><div style='font-size:12px; color:#8e8e93;'>{leg['market']} • {leg['price']}</div></div>", unsafe_allow_html=True)
